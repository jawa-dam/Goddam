...
const INTEL='geiProjectResearchDecisionOutcomeIntelligenceV682';
const OUTCOME='geiProjectResearchDecisionExecutionOutcomeV681';
const KEY='geiProjectResearchDecisionOutcomeDecisionV683';
...
function candidates(id){const i=latest(by(intelligence,id));if(!i)return [];return (MAP[i.interpretation]||[]).map(x=>({...x,artifactId:id,sourceIntelligenceId:i.id,sourceOutcomeId:i.sourceOutcomeId||'',interpretation:i.interpretation,strength:i.strength})).sort((a,b)=>(order[a.priority]??9)-(order[b.priority]??9)||b.score-a.score)}
function save(d){
 if(!d){alert('Select a ranked decision first.');return;}
 const note=String(el('note')?.value||'').trim();
 const rec={id:crypto.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`,artifactId:selected,sourceIntelligenceId:d.sourceIntelligenceId,sourceOutcomeId:d.sourceOutcomeId,decision:d.decision,priority:d.priority,score:d.score,route:d.route,status:'SELECTED',note,interpretation:d.interpretation,strength:d.strength,createdAt:new Date().toISOString(),lineage:'V6.69 → V6.70 → V6.71 → V6.72 → V6.73 → V6.23 → V6.74 → V6.75 → V6.76 → V6.77 → V6.78 → V6.79 → V6.80 → V6.81 → V6.82 → V6.83'};
 records=[...records,rec].slice(-500);localStorage.setItem(KEY,JSON.stringify(records));if(el('note'))el('note').value='';render();
}
function render(){
 const sel=el('artifact');sel.innerHTML=ids.length?ids.map(x=>`<option value="${esc(x)}" ${x===selected?'selected':''}>${esc(x)}</option>`).join(''):'<option value="">No V6.82 intelligence yet</option>';
 ...
 el('status').innerHTML=i?'<strong>DECISION ENGINE READY</strong><br>V6.82 intelligence is available and V6.83 can rank the next research decisions.':'<strong>AWAITING INTELLIGENCE</strong><br>Record V6.82 outcome intelligence before V6.83 can generate the next decision.';
 el('current').innerHTML=i?`<div class="item good"><span class="tag">${esc(i.priority)} • ${esc(i.strength)}</span><h3>${esc(i.interpretation)}</h3><p class="hint"><b>Outcome:</b> ${esc(i.outcome)}<br><b>Direction:</b> ${esc(i.direction)}<br><b>Route:</b> ${esc(i.route)}</p></div>`:'<p class="hint">No current V6.82 intelligence is available.</p>';
 ...
 el('history').innerHTML=hist.map(x=>`<div class="item"><span class="tag">${esc(x.status)} • ${esc(x.priority)} • ${esc(x.score)}</span><h3>${esc(x.decision)}</h3><p class="hint"><b>Interpretation:</b> ${esc(x.interpretation||'—')}<br>Route: ${esc(x.route)}<br><b>Note:</b> ${esc(x.note||'None')}<br>${new Date(x.createdAt||0).toLocaleString()}</p></div>`).join('')||'<p class="hint">No V6.83 decision history yet.</p>';
 el('trace').innerHTML=[['V6.82 Decision Outcome Intelligence',by(intelligence,selected).length],['V6.81 Decision Execution Outcomes',by(outcomes,selected).length],['V6.83 Decision Records',hist.length]].map(([label,n])=>`<div class="item ${n?'good':'warn'}"><b>${n?'✅':'⚠️'} ${esc(label)}</b><p class="hint">${n} record(s).</p></div>`).join('')+`<p class="hint"><strong>Lineage:</strong> V6.69 → V6.70 → V6.71 → V6.72 → V6.73 → V6.23 → V6.74 → V6.75 → V6.76 → V6.77 → V6.78 → V6.79 → V6.80 → V6.81 → V6.82 → V6.83</p>`;
 el('integrity').innerHTML='<p class="hint">V6.83 is deterministic, local-first, and researcher-controlled. It converts the fixed V6.82 outcome-intelligence classification into ranked process decisions. It does not establish scientific truth, causation, external validity, publication, peer review, acceptance, or real-world impact. V6.84 will orchestrate selected decisions.</p>';
}
