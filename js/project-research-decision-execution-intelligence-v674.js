...
const KEY='geiProjectResearchDecisionExecutionIntelligenceV674';
...
function save(a){
 ...
 const rec={
  ...
  lineage:'V6.69 → V6.70 → V6.71 → V6.72 → V6.73 → V6.23 → V6.74'
 };
...
function render(){
 ...
 el('status').innerHTML=ids.length?'<strong>INTELLIGENCE ENGINE READY</strong><br>V6.74 monitors the documented V6.73 handoff and V6.23 execution state for the selected research artifact.':'<strong>AWAITING HANDOFF</strong><br>Create a V6.73 execution handoff before V6.74 can evaluate decision execution.';
 el('current').innerHTML=item.h?`<div class="item ${tone}"><span class="tag">${esc(item.state)} • ${item.progress}%</span><h3>${esc(item.h.title)}</h3><p class="hint"><b>Handoff:</b> ${esc(item.h.status)}<br><b>Tasks:</b> ${item.done} / ${item.total} complete<br><b>Active:</b> ${item.active} • <b>Blocked:</b> ${item.blocked}<br><b>Route:</b> V6.23 Execution</p></div>`:'<p class="hint">No current V6.73 handoff found.</p>';
 el('signals').innerHTML=`<div class="item ${tone}"><h3>${esc(item.signal)}</h3><p class="hint">Execution state: <b>${esc(item.state)}</b> • Progress: <b>${item.progress}%</b></p><p class="hint">V6.74 interprets documented local execution records; it does not independently observe external activity.</p></div><textarea id="note" placeholder="Optional researcher note about this execution state…"></textarea><button id="record" class="btn">📝 RECORD CURRENT EXECUTION INTELLIGENCE</button>`;
 el('record').onclick=()=>save(item);
 el('history').innerHTML=hist.map(x=>`<div class="item"><span class="tag">${esc(x.state)} • ${esc(x.progress)}%</span><h3>${esc(x.title)}</h3><p class="hint">Signal: ${esc(x.signal)}<br>Tasks: ${x.completedTasks}/${x.totalTasks} complete • Active: ${x.activeTasks} • Blocked: ${x.blockedTasks}<br><b>Note:</b> ${esc(x.note||'None')}<br>${new Date(x.createdAt||0).toLocaleString()}</p></div>`).join('')||'<p class="hint">No V6.74 intelligence history yet.</p>';
 el('trace').innerHTML=[['V6.73 Execution Handoff',by(handoffs,selected).length],['V6.23 Execution Records',item.total],['Completed Tasks',item.done],['V6.74 Intelligence Records',hist.length]].map(([label,n])=>`<div class="item ${n?'good':'warn'}"><b>${n?'✅':'⚠️'} ${esc(label)}</b><p class="hint">${n} record(s).</p></div>`).join('')+`<p class="hint"><strong>Lineage:</strong> V6.69 → V6.70 → V6.71 → V6.72 → V6.73 → V6.23 → V6.74</p>`;
 el('integrity').innerHTML='<p class="hint">V6.74 is deterministic, local-first, and researcher-controlled. It summarizes documented V6.73 handoff and V6.23 execution records into decision-execution state, progress, and signals. It does not independently verify external execution, establish scientific truth, causation, external validity, publication, peer review, acceptance, or real-world impact. V6.75 will document the decision-execution outcome.</p>';
}
